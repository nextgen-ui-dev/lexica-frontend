...
const ArticleDetails = () => {
  ...
  const [articleDifficulty, setArticleDifficulty] = useState(DIFFICULTY.ADVANCE);
            ...
            <div className="flex flex-col items-center w-full h-full pt-8 md:pt-12">
              ...
              </div>
              {
                ...
                && <div className="w-full">
                  <h6 className="text-center mb-4">{data.source} {data.author ? `-- ${data.author}` : '-- Tidak diketahui'}</h6>
                  <h5 className="text-center mb-4">Pilih tingkatan literasi yang kamu minati</h5>
                  <div className="w-full grid grid-cols-3 mb-6">
                    <div
                      onClick={() => setArticleDifficulty(DIFFICULTY.ADVANCE)}
                      className={`col-span-1 cursor-pointer flex items-center justify-center border-b-2 py-2 ${articleDifficulty === DIFFICULTY.ADVANCE && 'border-primary-600'}`}>
                      <h3 className={`${articleDifficulty !== DIFFICULTY.ADVANCE && 'text-slate-400'}`}>Lanjutan</h3>
                    </div>
                    <div
                      onClick={() => setArticleDifficulty(DIFFICULTY.INTERMEDIATE)}
                      className={`col-span-1 cursor-pointer flex items-center justify-center border-b-2 py-2 ${articleDifficulty === DIFFICULTY.INTERMEDIATE && 'border-primary-600'}`}>
                      <h3 className={`${articleDifficulty !== DIFFICULTY.INTERMEDIATE && 'text-slate-400'}`}>Menengah</h3>
                    </div>
                    <div
                      onClick={() => setArticleDifficulty(DIFFICULTY.BEGINNER)}
                      className={`col-span-1 cursor-pointer flex items-center justify-center border-b-2 py-2 ${articleDifficulty === DIFFICULTY.BEGINNER && 'border-primary-600'}`}>
                      <h3 className={`${articleDifficulty !== DIFFICULTY.BEGINNER && 'text-slate-400'}`}>Pemula</h3>
                    </div>
                  </div>
                </div>
              }
